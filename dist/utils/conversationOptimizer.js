...
exports.formatConversationWithSummary = formatConversationWithSummary;
exports.extractKeyFacts = extractKeyFacts;
 ...
 */
function truncateMessages(messages, maxTokens = MAX_TOKENS_FOR_HISTORY, maxMessages = MAX_MESSAGES_FOR_HISTORY) {
 ...
 */
async function summarizeMessages(messages, genAI) {
    if (!messages || messages.length === 0) {
        return '';
    }
...
Summary:`;
            ...
            if (summary) {
                logger_1.logger.debug(`Generated AI summary for ${messages.length} messages`);
                return summary;
            }
        }
        catch (error) {
            logger_1.logger.warn('Failed to generate AI summary, using extraction method:', error.message);
        }
    }
 ...
 */
function formatConversationWithSummary(summary, recentMessages) {
    let conversation = '';
    if (summary) {
 ...
 */
function extractKeyFacts(messages, limit = 10) {
    const facts = [];
