...
exports.createPaystackSubscription = exports.handleWebhook = exports.verifyPayment = exports.initializePayment = void 0;
...
const handleWebhook = async (req, res) => {
    try {
        ...
        if (event.event === 'subscription.create' || event.event === 'subscription.update' || event.event === 'subscription.disable') {
            ...
            if (subCode) {
                const local = await Subscription_1.Subscription.findOne({ paystackSubscriptionCode: subCode });
                if (local) {
            ...
            if (reference) {
                const subscription = await Subscription_1.Subscription.findOne({ paystackReference: reference, status: 'pending' });
                if (subscription) {
...
const createPaystackSubscription = async (req, res) => {
    try {
...
exports.createPaystackSubscription = createPaystackSubscription;
