...
exports.getMoodEntriesWithCBT = exports.saveMoodEntryWithCBT = exports.getCBTAnalytics = exports.getCBTInsights = exports.generateAICBTInsights = exports.getCBTProgress = exports.getCBTActivities = exports.saveCBTActivity = exports.getThoughtRecords = exports.saveThoughtRecord = void 0;
...
const saveThoughtRecord = async (req, res) => {
    try {
        ...
        const thoughtRecord = new CBTThoughtRecord_1.CBTThoughtRecord({
            userId: req.user._id,
            situation,
            automaticThoughts,
            emotions: emotions || [],
        ...
        const activity = new CBTActivity_1.CBTActivity({
            userId: req.user._id,
            type: 'thought_record',
            data: {
...
exports.saveThoughtRecord = saveThoughtRecord;
// Get thought records
const getThoughtRecords = async (req, res) => {
    try {
        const limit = parseInt(req.query.limit) || 10;
        const offset = parseInt(req.query.offset) || 0;
        const thoughtRecords = await CBTThoughtRecord_1.CBTThoughtRecord.find({ userId: req.user._id })
            .sort({ createdAt: -1 })
            .limit(limit)
            .skip(offset);
        const total = await CBTThoughtRecord_1.CBTThoughtRecord.countDocuments({ userId: req.user._id });
        res.json({
            success: true,
            data: thoughtRecords,
            pagination: {
...
exports.getThoughtRecords = getThoughtRecords;
// Save CBT activity
const saveCBTActivity = async (req, res) => {
    try {
...
exports.saveCBTActivity = saveCBTActivity;
// Get CBT activities
const getCBTActivities = async (req, res) => {
    try {
        const limit = parseInt(req.query.limit) || 10;
        const offset = parseInt(req.query.offset) || 0;
        ...
        const activities = await CBTActivity_1.CBTActivity.find(query)
            .sort({ createdAt: -1 })
            .limit(limit)
            .skip(offset);
        const total = await CBTActivity_1.CBTActivity.countDocuments(query);
        res.json({
            success: true,
            data: activities,
            pagination: {
...
exports.getCBTActivities = getCBTActivities;
// Get CBT progress
const getCBTProgress = async (req, res) => {
    try {
        const userId = req.user._id;
        const now = new Date();
        const thirtyDaysAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
        // Get counts
        const thoughtRecordsCompleted = await CBTThoughtRecord_1.CBTThoughtRecord.countDocuments({
            userId,
            createdAt: { $gte: thirtyDaysAgo },
        });
        const activities = await CBTActivity_1.CBTActivity.find({
            userId,
            createdAt: { $gte: thirtyDaysAgo },
        });
        ...
        const moodEntries = await Mood_1.Mood.countDocuments({
            userId,
            createdAt: { $gte: thirtyDaysAgo },
        });
        // Calculate weekly streak
        const weekAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
        const recentActivities = await CBTActivity_1.CBTActivity.countDocuments({
            userId,
            createdAt: { $gte: weekAgo },
        });
        // Calculate goals achieved (thought records with balanced thoughts)
        const goalsAchieved = await CBTThoughtRecord_1.CBTThoughtRecord.countDocuments({
            userId,
            balancedThought: { $ne: "" },
            createdAt: { $gte: thirtyDaysAgo },
        });
        // Get latest activity
        const lastActivity = await CBTActivity_1.CBTActivity.findOne({ userId })
            .sort({ createdAt: -1 })
            .select('createdAt');
        res.json({
            success: true,
...
exports.getCBTProgress = getCBTProgress;
...
exports.generateAICBTInsights = generateAICBTInsights;
// Get CBT insights (data-driven insights from user's history)
const getCBTInsights = async (req, res) => {
    try {
        const userId = req.user._id;
        const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
        // Get cognitive distortions frequency
        const thoughtRecords = await CBTThoughtRecord_1.CBTThoughtRecord.find({
            userId,
            createdAt: { $gte: thirtyDaysAgo },
        });
...
exports.getCBTInsights = getCBTInsights;
// Get CBT analytics
const getCBTAnalytics = async (req, res) => {
    try {
        const userId = req.user._id;
...
exports.getCBTAnalytics = getCBTAnalytics;
// Save mood entry with CBT insights
const saveMoodEntryWithCBT = async (req, res) => {
    try {
...
exports.saveMoodEntryWithCBT = saveMoodEntryWithCBT;
// Get mood entries with CBT data
const getMoodEntriesWithCBT = async (req, res) => {
    try {
        const limit = parseInt(req.query.limit) || 10;
        const offset = parseInt(req.query.offset) || 0;
        const period = req.query.period || '30days';
        ...
        const moodEntries = await Mood_1.Mood.find({
            ...
            .limit(limit)
            .skip(offset);
        const total = await Mood_1.Mood.countDocuments({
            userId: req.user._id,
            createdAt: { $gte: startDate },
        });
        res.json({
...
exports.getMoodEntriesWithCBT = getMoodEntriesWithCBT;
