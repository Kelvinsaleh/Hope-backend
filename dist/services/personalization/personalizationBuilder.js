...
exports.buildEnforcementRules = buildEnforcementRules;
exports.buildUserProfileSummary = buildUserProfileSummary;
exports.trackEngagementSignal = trackEngagementSignal;
const mongoose_1 = require("mongoose");
const Personalization_1 = require("../../models/Personalization");
const logger_1 = require("../../utils/logger");
const conversationSummarization_1 = require("./conversationSummarization");
/**
...
async function buildPersonalizationContext(userId, includeSummaries = true) {
    try {
        ...
        if (!personalization) {
            logger_1.logger.debug(`No personalization found for user ${userId}, using defaults`);
            return null;
        }
        ...
        if (includeSummaries) {
            const summaries = await (0, conversationSummarization_1.getRecentSummaries)(userId, 4); // Last 4 summaries
        ...
        logger_1.logger.error(`Error building personalization context for user ${userId}:`, error);
        return null;
    }
}
/**
 ...
 */
function applyDecay(personalization) {
    const now = Date.now();
    ...
    if (personalization.behavioralTendencies && Array.isArray(personalization.behavioralTendencies)) {
        personalization.behavioralTendencies = personalization.behavioralTendencies
            ...
            .filter((tendency) => tendency.confidence > 0.2) // Remove very low-confidence patterns
    ...
    if (personalization.adaptationRules && Array.isArray(personalization.adaptationRules)) {
        personalization.adaptationRules = personalization.adaptationRules
            ...
            .filter((rule) => rule.confidence > 0.3) // Remove low-confidence rules
 ...
 */
function buildEnforcementRules(context) {
    if (!context) {
 ...
 */
function buildUserProfileSummary(context) {
    if (!context) {
        return "";
    }
 ...
 */
async function trackEngagementSignal(userId, signal) {
    try {
        const personalization = await Personalization_1.Personalization.findOne({
            userId: new mongoose_1.Types.ObjectId(userId),
        });
        ...
        logger_1.logger.debug(`Updated engagement metrics for user ${userId}`);
    }
    catch (error) {
        logger_1.logger.error(`Error tracking engagement signal for user ${userId}:`, error);
