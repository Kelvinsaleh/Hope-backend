...
import { Personalization, IPersonalization, IBehavioralTendency, IAdaptationRule } from "../../models/Personalization";
 ...
 * Extracts behavioral patterns from recent interactions
 * Updates personalization data only when patterns are consistent
 */

interface InteractionPattern {
  type: "communication_style" | "verbosity" | "topic_preference" | "engagement" | "time_pattern";
  evidence: string[];
  confidence: number;
  frequency: number;
  sampleSize: number;
 ...
 * Analyze communication style from messages
 */
function analyzeCommunicationStyle(sessions: any[]): InteractionPattern | null {
  ...
  for (const msg of userMessages) {
    ...
    if (length < 50) shortMessages++;
    if (length > 200) longMessages++;
  ...
  let inferredStyle: "gentle" | "direct" | "supportive" | null = null;
  ...
  if (!inferredStyle || evidence.length === 0) return null;

  const confidence = Math.min(
    0.9,
    0.5 + (total / 50) * 0.4 // Higher confidence with more data
  );
 ...
 * Analyze verbosity preferences
 */
function analyzeVerbosity(sessions: any[]): InteractionPattern | null {
  const allMessages = sessions.flatMap(s => s.messages || []);
  if (allMessages.length < 5) return null;
  ...
  let inferredVerbosity: "concise" | "moderate" | "detailed" | null = null;
 ...
 * Analyze topic preferences
 */
function analyzeTopicPreferences(sessions: any[], journals: any[]): InteractionPattern | null {
  ...
  if (preferredTopics.length === 0) return null;
 ...
 * Analyze engagement patterns
 */
function analyzeEngagement(sessions: any[]): InteractionPattern | null {
...
export async function analyzeTimePatterns(
  ...
  try {
    ...
    const typicalRange: [number, number] = durations.length > 0
      ? [
 ...
 */
export async function updatePersonalizationFromPatterns(
  userId: string,
  patterns: InteractionPattern[],
  timeAnalysis: TimeAnalysis
): Promise<void> {
  try {
    let personalization = await Personalization.findOne({ userId: new Types.ObjectId(userId) });
    ...
    logger.error("Error updating personalization from patterns:", error);
    throw error;
  }
}
